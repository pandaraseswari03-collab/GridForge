...
import core.analysis.contingency as contingency_module
...
def test_prepared_equipment_preserves_engineering_ratings():
    branch = PreparedBranch("L1", "B1", "B2", 0.01, 0.1, 0.02, rate_mva=50.0, rated_current_a=200.0, thermal_limit_mva=45.0)
    transformer = PreparedTransformer("T1", "B1", "B2", 0.01, 0.08, 0.02, 1.05, 0.1, rated_mva=40.0)
...
def test_transformer_flow_uses_the_same_terminal_admittance_as_ybus():
    transformer = PreparedTransformer("T1", "B1", "B2", 0.01, 0.08, 0.04, 1.05, math.radians(7.0))
    prepared = _prepared(transformers=(transformer,))
    V = np.array([1.02 * np.exp(1j * 0.03), 0.98 * np.exp(-1j * 0.02)])
    result = TransformerFlowCalculator.from_prepared(prepared).calculate_prepared(transformer, np.abs(V), np.angle(V))
...
def test_engineering_converter_exposes_branch_and_transformer_results():
    numerical = PowerFlowResult(success=True, iterations=3, error=1e-9, pv_to_pq=(), history=(1.0, 1e-3, 1e-9), message="Converged", voltage_magnitudes=(1.0, 0.98), voltage_angles=(0.0, -0.02))
    prepared = _prepared(
...
def test_power_flow_result_engineering_payload_is_immutable():
    numerical = PowerFlowResult(success=True, iterations=0, error=0.0, pv_to_pq=(), history=(), message="ok", voltage_magnitudes=(1.0,), voltage_angles=(0.0,))
...
def test_contingency_violations_use_stable_id_engineering_results():
    ...
    class _Bus:
        def __init__(self, bus_id): self.id = bus_id
    class _Network:
        buses = [_Bus("B1"), _Bus("B2")]
        lines = []
        transformers = []
    result = PowerFlowResult(success=True, iterations=1, error=0.0, pv_to_pq=(), history=(0.0,), message="Converged", voltage_magnitudes=(1.0, 0.98), voltage_angles=(0.0, 0.0), bus_results={"B2": {"voltage_magnitude": 0.90}, "B1": {"voltage_magnitude": 1.10}}, branch_results={"L-42": {"loading_percent": 125.0, "limit_mva": 80.0, "within_limit": False}}, transformer_results={"T-7": {"loading_percent": 110.0, "limit_mva": 100.0, "within_limit": False}})
    violations = analysis._detect_violations(_Network(), result, voltage_min=0.95, voltage_max=1.05, thermal_limit=100.0)
    assert [(v.category, v.element_id) for v in violations] == [("voltage_low", "B2"), ("voltage_high", "B1"), ("thermal", "L-42"), ("transformer_thermal", "T-7")]
...
def test_power_flow_analysis_keeps_engineering_results_on_same_result_contract(monkeypatch):
    ...
    numerical = PowerFlowResult(success=True, iterations=1, error=0.0, pv_to_pq=(), history=(0.0,), message="Converged", voltage_magnitudes=(1.0, 0.98), voltage_angles=(0.0, -0.02))
...
def test_contingency_failed_power_flow_is_a_failed_case(monkeypatch):
    prepared = _prepared()
    failed_result = PowerFlowResult(success=False, iterations=10, error=1.0, pv_to_pq=(), history=(1.0,), message="Maximum iterations reached.", voltage_magnitudes=(0.80, 1.20), voltage_angles=(0.0, 0.0))

    class _Preparation:
        def __init__(self, network, configuration): pass
        def prepare(self): return prepared

    class _PowerFlow:
        def __init__(self, *args, **kwargs): pass
        def solve(self): return failed_result

    analysis = ContingencyAnalysis.__new__(ContingencyAnalysis)
    analysis.power_flow_configuration = object()
    analysis._create_outage_case = lambda outages: object()
    monkeypatch.setattr(contingency_module, "PowerFlowPreparation", _Preparation)
    monkeypatch.setattr(contingency_module, "PowerFlowAnalysis", _PowerFlow)
    case = analysis._run_case(outages=("L1",), power_flow_options=None, voltage_min=0.95, voltage_max=1.05, thermal_limit=100.0)
    assert case.success is False
    assert case.converged is False
    assert case.violations == []
    assert case.power_flow_result is failed_result


def test_contingency_does_not_fall_back_to_positional_voltage_identity():
    analysis = ContingencyAnalysis.__new__(ContingencyAnalysis)
    class _Bus:
        def __init__(self, bus_id): self.id = bus_id
    class _Network:
        buses = [_Bus("B2"), _Bus("B1")]
        lines = []
        transformers = []
    result = PowerFlowResult(success=True, iterations=1, error=0.0, pv_to_pq=(), history=(0.0,), message="Converged", voltage_magnitudes=(0.90, 1.10), voltage_angles=(0.0, 0.0))
    violations = analysis._detect_violations(_Network(), result, voltage_min=0.95, voltage_max=1.05, thermal_limit=100.0)
    assert violations == []
